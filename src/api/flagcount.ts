...
export const flagcountApi: FlagCountApi = {
  ...
  connect: (username: string, platform): Promise<void> => invoke('connect', platform ? { username, platform } : { username }),
  ...
  setLiveScene: (sceneId: string): Promise<void> => invoke('set_live_scene', { sceneId }),
  setLiveHidden: (hidden: boolean): Promise<void> => invoke('set_live_hidden', { hidden }),
