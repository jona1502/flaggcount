...
describe('Zähler & Abstimmungen', () => {
  ...
  it('asks before leaving the page with unsaved changes', async () => {
    ...
    await user.click(within(navigation).getByRole('button', { name: 'Live-Steuerung' }));
    ...
    expect(screen.getByRole('heading', { level: 1 }).textContent).toBe('Live-Steuerung');
  });
  ...
  it('creates a poll from the overview shortcut and shows where it appears', async () => {
    ...
    await user.click(within(created).getByRole('button', { name: 'Live-Steuerung öffnen' }));
    expect(screen.getByRole('heading', { level: 1 }).textContent).toBe('Live-Steuerung');
  });
