...
export function HistoryPanel({ records, available, disabled, onClear }: { records: RoundRecord[]; available: boolean; disabled: boolean; onClear: () => void }): React.JSX.Element {
  ...
  return <section className="panel" aria-labelledby="history-heading">
    <div className="panel-heading"><h2 id="history-heading">Rundenhistorie</h2><button type="button" className="button secondary" disabled={disabled || records.length === 0} onClick={() => { if (window.confirm('Gesamte lokale Historie wirklich löschen?')) onClear(); }}>Historie löschen</button></div>
